...
class VideoComponentPage(VideoPage):
    ...
    def upload_asset(self, asset_filename, asset_type='handout', index=0):
        ...
        self.click_button('upload_asset', index=index)
    ...
    def download_translation(self, language_code, text_to_search):
        """
        Download a translation having `language_code` and contains `text_to_search`
        ...
        Returns:
            bool: result of download text_to_search in translation
    ...
    def is_transcript_button_visible(self, button_name, index=0, button_text=None):
        ...
        Arguments:
            ...
            button_text (str): text displayed on a button

        Returns:
            bool: is button visible

        """
        button_text_result = True
        if button_text:
            button_text_result = button_text == self.q(css=BUTTON_SELECTORS[button_name]).nth(index).text[0]

        return self.q(css=BUTTON_SELECTORS[button_name]).nth(index).visible and button_text_result
