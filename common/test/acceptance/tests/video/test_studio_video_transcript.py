...
class VideoTranscriptTest(CMSVideoBaseTest):
    ...
    def test_set_youtube_id_wo_server(self):
        ...
        self.video.set_url_field('test_video_name.mp4', 2)
        self.assertEqual(self.video.message('status'), 'Timed Transcript Found')
        self.open_advanced_tab()
        self.assertTrue(self.video.verify_field_value('Default Timed Transcript', 't_not_exist'))
    ...
    def test_set_youtube_id_wo_local(self):
        """
        Scenario: User sets youtube_id w/o local but with server subs and one html5 link w/o transcripts w/o
                  import action, then another one html5 link w/o transcripts
        Given I have created a Video component
        And I edit the component
    ...
    def test_two_html5_sources_w_transcripts(self):
        ...
        And I see button "Timed Transcript from uk_transcripts.mp4"
        And I see button "Timed Transcript from t_not_exist.webm"
    ...
    def test_upload_subtitles_w_different_names(self):
        """
    ...
    def test_upload_subtitles_w_different_names2(self):
        """
        Uploading subtitles for file with periods in it should properly set the transcript name and keep the periods
